# rapacl/engines/_train_scratch_with_mmcl_recon_cls__eval_genepred.py
...
def set_stage1_trainable(model: nn.Module, recon_only: bool = False) -> None:
    """
    Stage 1 training policy.

    Warmup(recon_only=True):
      - train radiomics TransTab + projection head + recon head
      - freeze pathomics encoder, pathomics projection, cls head, gene head

    Full Stage1(recon_only=False):
      - train radiomics TransTab + projection head, pathomics projection, recon head, cls head
      - freeze pathomics encoder and gene head
    """
    freeze_module(model.pathomics_encoder)

    set_requires_grad(model.radiomics_model, True)
    set_requires_grad(model.recon_head, True)

    set_requires_grad(model.pathomics_proj, not recon_only)
    set_requires_grad(model.cls_head, not recon_only)
    set_requires_grad(model.gene_head, False)

    model.radiomics_model.train()
    model.recon_head.train()
    model.pathomics_proj.train(not recon_only)
    model.cls_head.train(not recon_only)
    model.gene_head.eval()
...
class MMCLReconClsModel(nn.Module):
    ...
    def encode_pathomics_projected(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        with torch.no_grad():
    ...
    def forward_pretrain(self, image: torch.Tensor, radiomics: torch.Tensor | pd.DataFrame):
        ...
        path = self.encode_pathomics_projected(image)
...
def train_pretrain_epoch(model, loader, optimizer, device, recon_only: bool = False):
    set_stage1_trainable(model, recon_only=recon_only)
    ...
    desc = "stage1_recon_warmup_train" if recon_only else "stage1_train"
    ...
    for batch in tqdm(loader, desc=desc, leave=False):
        ...
        if recon_only:
            ...
            acc_value = accuracy(out["pred_class_logits"].detach(), target_label)
        ...
        meter["acc"] += acc_value * bs

    n = len(loader.dataset)
    return {k: v / n for k, v in meter.items()}
@torch.no_grad()
def eval_pretrain_epoch(model, loader, device, recon_only: bool = False):
    model.eval()
    temperature = getattr(constants, "CONTRASTIVE_TEMPERATURE", 0.07)
    meter = {"loss": 0.0, "mmcl": 0.0, "recon": 0.0, "cls": 0.0, "acc": 0.0}
    mmcl_w = getattr(constants, "MMCL_LAMBDA", MMCL_LAMBDA)
    recon_w = getattr(constants, "RECON_LAMBDA", RECON_LAMBDA)
    cls_w = getattr(constants, "CLS_LAMBDA", CLS_LAMBDA)
    desc = "stage1_recon_warmup_val" if recon_only else "stage1_val"
    ...
    for batch in tqdm(loader, desc=desc, leave=False):
        ...
        if recon_only:
            ...
            cls_loss = F.cross_entropy(out["pred_class_logits"], target_label)
            loss = mmcl_w * mmcl_loss + recon_w * recon_loss + cls_w * cls_loss
            acc_value = accuracy(out["pred_class_logits"], target_label)
...
def main():
    ...
    # If PRETRAIN_EPOCHS <= WARMUP_RECON_EPOCHS, fall back to the recon-only checkpoint.
    ...
    optimizer_stage2 = torch.optim.AdamW(
        model.gene_head.parameters(),
        lr=getattr(constants, "GENE_LR", 1e-4),
