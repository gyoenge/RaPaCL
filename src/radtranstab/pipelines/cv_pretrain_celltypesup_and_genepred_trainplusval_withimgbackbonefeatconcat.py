...
class Config:
    ...
    image_feature_cache_dirname: str = "image_feature_cache"
    image_feature_dim: int = 1024
    concat_hidden_dim: int = 512
...
def build_config(cfg_dict: dict) -> Config:
    ...
    return Config(
        ...
        include_val_in_train=bool(train_cfg.get("include_val_in_train", False)),
        ...
        image_batch_size=int(image_cfg.get("batch_size", 128)),
        image_num_workers=int(image_cfg.get("num_workers", 0)),
        densenet_weights=str(image_cfg.get("weights", "IMAGENET1K_V1")),
        freeze_image_backbone=bool(image_cfg.get("freeze_backbone", True)),
        image_feature_cache_dirname=str(image_cfg.get("cache_dirname", "image_feature_cache")),
        image_feature_dim=int(image_cfg.get("feature_dim", 1024)),
        concat_hidden_dim=int(image_cfg.get("concat_hidden_dim", 512)),
    )
...
class DenseNetFeatureExtractor(nn.Module):
    def __init__(self, cfg: Config, device: str) -> None:
        super().__init__()
        weights = None
        if cfg.densenet_weights and cfg.densenet_weights.upper() != "NONE":
            weights = getattr(models.DenseNet121_Weights, cfg.densenet_weights)
...
def pretrain_one_fold(
    ...
    for epoch in range(cfg.max_epochs):
        ...
        selection_metric = tr["total_loss"] if cfg.include_val_in_train else va["total_loss"]
    ...
    with open(pretrain_dir / "pretrain_summary.json", "w", encoding="utf-8") as f:
        json.dump({"best_epoch": best_epoch, "best_selection_metric": best_metric}, f, indent=2)


@torch.no_grad()
def compute_transtab_embeddings_df(
...
def _find_image_tensor_key(chunk: dict) -> str:
    candidates = ["imgs", "images", "patches", "img", "image"]
    for key in candidates:
        if key in chunk:
            return key
    raise KeyError(f"Could not find image tensor key in chunk keys={list(chunk.keys())}")


def _prepare_image_tensor(x: torch.Tensor) -> torch.Tensor:
...
def build_or_load_image_feature_cache(
    ...
    cache_path = cache_root / f"{sample_id}_densenet121_features.parquet"
    requested_set = set(requested_barcodes)

    if cache_path.exists():
        cached = pd.read_parquet(cache_path)
        cached["barcode"] = cached["barcode"].astype(str)
        cached = cached[cached["barcode"].isin(requested_set)].reset_index(drop=True)
        if not cached.empty:
            return cached

    extractor = DenseNetFeatureExtractor(cfg, device)
    patches_h5_path = os.path.join(cfg.bench_data_root, "patches", f"{sample_id}.h5")
    patch_dataset = H5PatchDataset(patches_h5_path)

    rows = []
    for i in range(len(patch_dataset)):
        chunk = patch_dataset[i]
        image_key = _find_image_tensor_key(chunk)
        imgs = chunk[image_key]
        barcodes = chunk["barcodes"]

        if isinstance(barcodes, torch.Tensor):
            barcodes = barcodes.cpu().numpy().tolist()
        if isinstance(barcodes, (bytes, str)):
            barcodes = [barcodes]
        barcodes = [b.decode("utf-8") if isinstance(b, bytes) else str(b) for b in barcodes]

        imgs = _prepare_image_tensor(imgs)
        if imgs.shape[0] != len(barcodes):
            raise ValueError(f"Image count and barcode count mismatch for sample_id={sample_id}, chunk={i}")

        keep_indices = [idx for idx, bc in enumerate(barcodes) if bc in requested_set]
        if not keep_indices:
            continue

        imgs = imgs[keep_indices]
        keep_barcodes = [barcodes[idx] for idx in keep_indices]

        for start in range(0, len(keep_barcodes), cfg.image_batch_size):
            sub_imgs = imgs[start:start + cfg.image_batch_size]
            sub_barcodes = keep_barcodes[start:start + cfg.image_batch_size]
            feats = extractor(sub_imgs).detach().cpu().numpy().astype(np.float32)
            for bc, feat in zip(sub_barcodes, feats):
                row = {"barcode": bc}
                row.update({f"img_feat_{j}": float(feat[j]) for j in range(feat.shape[0])})
                rows.append(row)

    if not rows:
        raise ValueError(f"No image features were extracted for sample_id={sample_id}")

    out_df = pd.DataFrame(rows).drop_duplicates(subset=["barcode"]).reset_index(drop=True)
    full_out = out_df.copy()
    if not cache_path.exists():
        full_out.to_parquet(cache_path, index=False)
    return out_df


def compute_densenet_features_for_df(df: pd.DataFrame, cfg: Config, output_dir: Path, device: str) -> np.ndarray:
    cache_root = ensure_dir(output_dir / cfg.image_feature_cache_dirname)
    feature_dfs = []
    needed_cols = [cfg.sample_id_col, cfg.barcode_col]
    uniq = df[needed_cols].drop_duplicates().reset_index(drop=True)
    for sample_id, sub in uniq.groupby(cfg.sample_id_col):
        barcodes = sub[cfg.barcode_col].astype(str).tolist()
        feat_df = build_or_load_image_feature_cache(str(sample_id), barcodes, cfg, cache_root, device)
        feat_df[cfg.sample_id_col] = str(sample_id)
        feature_dfs.append(feat_df)

    img_df = pd.concat(feature_dfs, axis=0, ignore_index=True)
    img_df[cfg.barcode_col] = img_df["barcode"].astype(str)

    merge_cols = [cfg.sample_id_col, cfg.barcode_col]
    merged = df[[cfg.sample_id_col, cfg.barcode_col]].copy()
    merged[cfg.sample_id_col] = merged[cfg.sample_id_col].astype(str)
    merged[cfg.barcode_col] = merged[cfg.barcode_col].astype(str)
    img_df[cfg.sample_id_col] = img_df[cfg.sample_id_col].astype(str)

    merged = merged.merge(
        img_df.drop(columns=["barcode"]),
        how="left",
        on=merge_cols,
    )
    feat_cols = [c for c in merged.columns if c.startswith("img_feat_")]
    if not feat_cols:
        raise ValueError("No image feature columns found after merge.")
    if merged[feat_cols].isna().any().any():
        missing = int(merged[feat_cols].isna().any(axis=1).sum())
        raise ValueError(f"Image feature merge produced missing rows: {missing}")
    return merged[feat_cols].to_numpy(dtype=np.float32)
...
def fit_fold(
    ...
    if cfg.include_val_in_train:
        effective_train_df = pd.concat([train_df, val_df], axis=0, ignore_index=True)
        effective_pretrain_val_df = effective_train_df
    else:
        effective_train_df = train_df
        effective_pretrain_val_df = val_df
    ...
    print(f"[fold {fold_id}] extracting pretrained tabular embeddings...")
    extractor = PretrainedEmbeddingExtractor(feature_cols, str(pretrain_dir), cfg, device)

    train_tab = compute_transtab_embeddings_df(extractor, effective_train_df, feature_cols, cfg.eval_batch_size)
    val_tab = compute_transtab_embeddings_df(extractor, val_df, feature_cols, cfg.eval_batch_size)
    test_tab = compute_transtab_embeddings_df(extractor, test_df, feature_cols, cfg.eval_batch_size)

    print(f"[fold {fold_id}] extracting densenet image features...")
    train_img = compute_densenet_features_for_df(effective_train_df, cfg, fold_dir, device)
    val_img = compute_densenet_features_for_df(val_df, cfg, fold_dir, device)
    test_img = compute_densenet_features_for_df(test_df, cfg, fold_dir, device)

    train_emb = np.concatenate([train_tab, train_img], axis=1)
    val_emb = np.concatenate([val_tab, val_img], axis=1)
    test_emb = np.concatenate([test_tab, test_img], axis=1)

    train_y = effective_train_df[gene_cols].to_numpy(dtype=np.float32)
    val_y = val_df[gene_cols].to_numpy(dtype=np.float32)
    test_y = test_df[gene_cols].to_numpy(dtype=np.float32)

    train_loader = DataLoader(
        EmbeddingGeneDataset(train_emb, train_y),
        batch_size=cfg.batch_size,
        shuffle=True,
        num_workers=cfg.num_workers,
        pin_memory=False,
    ...
    val_loader = DataLoader(
        EmbeddingGeneDataset(val_emb, val_y),
        batch_size=cfg.eval_batch_size,
        shuffle=False,
        num_workers=cfg.num_workers,
        pin_memory=False,
    ...
    test_loader = DataLoader(
        EmbeddingGeneDataset(test_emb, test_y),
        batch_size=cfg.eval_batch_size,
        shuffle=False,
        num_workers=cfg.num_workers,
        pin_memory=False,
    ...
    gene_head = GeneHead(
        ...
        hidden_dim=cfg.concat_hidden_dim,
        out_dim=len(gene_cols),
        dropout=cfg.dropout,
    ).to(device)
    ...
    best_epoch = -1
    patience_count = 0
    best_ckpt_path = fold_dir / "best_concat_gene_head.pt"
    history = []
    ...
    for epoch in range(cfg.max_epochs):
        ...
        selection_metric = train_loss if cfg.include_val_in_train else val_loss
        if selection_metric < best_metric:
            best_metric = selection_metric
            best_epoch = epoch
            patience_count = 0
            torch.save(
                {
                    ...
                    "image_dim": int(train_img.shape[1]),
                    "concat_dim": int(train_emb.shape[1]),
                    "gene_cols": gene_cols,
                },
                best_ckpt_path,
            )
    ...
    pd.DataFrame(history).to_csv(fold_dir / "concat_gene_head_history.csv", index=False)
    ...
    np.save(fold_dir / "train_tab_emb.npy", train_tab)
    np.save(fold_dir / "val_tab_emb.npy", val_tab)
    np.save(fold_dir / "test_tab_emb.npy", test_tab)
    np.save(fold_dir / "train_img_emb.npy", train_img)
    np.save(fold_dir / "val_img_emb.npy", val_img)
    np.save(fold_dir / "test_img_emb.npy", test_img)
    np.save(fold_dir / "train_concat_emb.npy", train_emb)
    np.save(fold_dir / "val_concat_emb.npy", val_emb)
    np.save(fold_dir / "test_concat_emb.npy", test_emb)
    ...
    summary = {
        ...
        "tabular_embedding_dim": int(train_tab.shape[1]),
        "image_embedding_dim": int(train_img.shape[1]),
        "concat_embedding_dim": int(train_emb.shape[1]),
        **{f"val_{k}": v for k, v in val_metrics.items()},
        **{f"test_{k}": v for k, v in test_metrics.items()},
        "n_train": len(effective_train_df),
        "n_val": len(val_df),
        "include_val_in_train": cfg.include_val_in_train,
        "n_test": len(test_df),
    }
...
def main() -> None:
    ...
    for fold_id, fold in enumerate(folds):
        ...
        effective_n_train = len(train_df) + len(val_df) if cfg.include_val_in_train else len(train_df)
        print(f"[fold {fold_id}] n_train={len(train_df)} n_val={len(val_df)} n_test={len(test_df)} effective_train={effective_n_train}")
