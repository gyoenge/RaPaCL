...
class Config:
    ...
    aux_target_prefixes: Optional[list[str]] = None
    aux_target_exact_cols: Optional[list[str]] = None
    ...
    # multitask pretrain using auxiliary feature targets
    pretrain_aux_loss_weight: float = 0.1
    pretrain_aux_warmup_epochs: int = 0
    pretrain_temperature: float = 0.1
    pretrain_aux_use_both_views: bool = True
    pretrain_checkpoint_name: str = "pretrain_multitask_aux.pt"
...
def build_config(cfg_dict: dict) -> Config:
    ...
    return Config(
        ...
        aux_target_prefixes=data.get("aux_target_prefixes", ["dist_", "n_cells_total"]),
        aux_target_exact_cols=data.get("aux_target_exact_cols", []),
        ...
        pretrain_aux_loss_weight=float(train_cfg.get("pretrain_aux_loss_weight", 0.1)),
        pretrain_aux_warmup_epochs=int(train_cfg.get("pretrain_aux_warmup_epochs", 0)),
        pretrain_temperature=float(train_cfg.get("pretrain_temperature", 0.1)),
        pretrain_aux_use_both_views=bool(train_cfg.get("pretrain_aux_use_both_views", True)),
        pretrain_checkpoint_name=str(train_cfg.get("pretrain_checkpoint_name", "pretrain_multitask_aux.pt")),
    )
...
def infer_aux_target_columns(all_feature_cols: list[str], cfg: Config) -> list[str]:
    prefixes = tuple((cfg.aux_target_prefixes or []))
    exact_cols = {c.lower() for c in (cfg.aux_target_exact_cols or [])}

    aux_cols = []
    for col in all_feature_cols:
        lowered = col.lower()
        if lowered in exact_cols or any(lowered.startswith(prefix.lower()) for prefix in prefixes):
            aux_cols.append(col)

    if not aux_cols:
        raise ValueError(
            "No auxiliary target columns matched. "
            f"aux_target_prefixes={cfg.aux_target_prefixes}, aux_target_exact_cols={cfg.aux_target_exact_cols}"
        )
    return aux_cols



def split_input_and_aux_feature_columns(all_feature_cols: list[str], cfg: Config) -> tuple[list[str], list[str]]:
    aux_target_cols = infer_aux_target_columns(all_feature_cols, cfg)
    aux_set = set(aux_target_cols)
    input_feature_cols = [c for c in all_feature_cols if c not in aux_set]

    if not input_feature_cols:
        raise ValueError("All feature columns became auxiliary targets; no input features remain.")

    return input_feature_cols, aux_target_cols
...
def build_joint_dataframe(cfg: Config) -> tuple[pd.DataFrame, list[str], list[str], list[str], list[str]]:
    ...
    all_feature_cols = feature_cols_ref
    input_feature_cols, aux_target_cols = split_input_and_aux_feature_columns(all_feature_cols, cfg)
    gene_cols = genes

    keep_cols = all_feature_cols + gene_cols
    print("before dropna:", len(joint_df))
    print("rows with any input feature NaN:", joint_df[input_feature_cols].isna().any(axis=1).sum())
    print("rows with any aux target NaN:", joint_df[aux_target_cols].isna().any(axis=1).sum())
    print("rows with any gene NaN:", joint_df[gene_cols].isna().any(axis=1).sum())
    print("rows with any feature/gene NaN:", joint_df[keep_cols].isna().any(axis=1).sum())
    print(joint_df[keep_cols].isna().sum().sort_values(ascending=False).head(30))
    ...
    print(f"[build_joint_dataframe] num_all_features={len(all_feature_cols)}")
    print(f"[build_joint_dataframe] num_input_features={len(input_feature_cols)}")
    print(f"[build_joint_dataframe] num_aux_targets={len(aux_target_cols)}")
    print(f"[build_joint_dataframe] num_genes={len(gene_cols)}")
    return joint_df, all_feature_cols, input_feature_cols, aux_target_cols, gene_cols
...
class TabularAuxDataset(Dataset):
    def __init__(self, df: pd.DataFrame, input_feature_cols: list[str], aux_target_cols: list[str]) -> None:
        self.x = df[input_feature_cols].reset_index(drop=True).copy()
        self.y = df[aux_target_cols].to_numpy(dtype=np.float32)
...
class EmbeddingTargetDataset(Dataset):
...
class MultiTaskPretrainModel(nn.Module):
    def __init__(self, base_model: nn.Module, aux_out_dim: int, hidden_dim: int, dropout: float) -> None:
        super().__init__()
        self.base_model = base_model
        self.aux_head = MLPHead(in_dim=hidden_dim, hidden_dim=hidden_dim, out_dim=aux_out_dim, dropout=dropout)
    ...
    def aux_predict(self, cls_emb: torch.Tensor) -> torch.Tensor:
        return self.aux_head(cls_emb)


class PretrainedEmbeddingExtractor(nn.Module):
    def __init__(self, input_feature_cols: list[str], checkpoint_dir: str, cfg: Config, device: str) -> None:
        super().__init__()
        self.feature_cols = input_feature_cols
        self.device_name = device
        self.model, _ = build_contrastive_learner(
            categorical_columns=[],
            numerical_columns=input_feature_cols,
            binary_columns=[],
...
def run_multitask_pretrain_epoch(
    ...
    for x_df, y_aux in loader:
        x_df = x_df[input_feature_cols].reset_index(drop=True)
        y_aux = y_aux.to(device)

        view1 = make_masked_view(x_df, input_feature_cols, cfg.num_partition, cfg.overlap_ratio)
        view2 = make_masked_view(x_df, input_feature_cols, cfg.num_partition, cfg.overlap_ratio)
        ...
        if cfg.pretrain_aux_use_both_views:
            aux_pred = 0.5 * (model.aux_predict(cls1) + model.aux_predict(cls2))
        else:
            aux_pred = model.aux_predict(cls1)
        loss_aux = F.mse_loss(aux_pred, y_aux)

        aux_weight = cfg.pretrain_aux_loss_weight if epoch >= cfg.pretrain_aux_warmup_epochs else 0.0
        loss = loss_ctr + aux_weight * loss_aux
        ...
        total_aux += float(loss_aux.item()) * bs
        total_count += bs
...
def save_pretrain_checkpoint(
    ...
    payload = {
        ...
        "aux_head_state_dict": model.aux_head.state_dict(),
...
def pretrain_one_fold(
    ...
    model = MultiTaskPretrainModel(
        ...
        aux_out_dim=len(aux_target_cols),
        hidden_dim=cfg.embedding_dim,
        dropout=cfg.dropout,
    ).to(device)

    train_loader = DataLoader(
        TabularAuxDataset(train_df, input_feature_cols, aux_target_cols),
    ...
    val_loader = DataLoader(
        TabularAuxDataset(val_df, input_feature_cols, aux_target_cols),
    ...
    for epoch in range(cfg.max_epochs):
        tr = run_multitask_pretrain_epoch(model, train_loader, input_feature_cols, cfg, device, epoch, optimizer)
        va = run_multitask_pretrain_epoch(model, val_loader, input_feature_cols, cfg, device, epoch, None)
        row = {
            "epoch": epoch,
            "train_total_loss": tr["total_loss"],
            "train_contrastive_loss": tr["contrastive_loss"],
            "train_aux_loss": tr["aux_loss"],
            "val_total_loss": va["total_loss"],
            "val_contrastive_loss": va["contrastive_loss"],
            "val_aux_loss": va["aux_loss"],
        }
        history.append(row)
        print(
            f"[pretrain] epoch={epoch:03d} "
            f"train_total={tr['total_loss']:.6f} train_ctr={tr['contrastive_loss']:.6f} train_aux={tr['aux_loss']:.6f} "
            f"val_total={va['total_loss']:.6f} val_ctr={va['contrastive_loss']:.6f} val_aux={va['aux_loss']:.6f}"
        )
        ...
        if va["total_loss"] < best_val:
            ...
            best_epoch = epoch
            patience_count = 0
            save_pretrain_checkpoint(model, pretrain_dir, cfg, input_feature_cols, aux_target_cols)
        else:
            patience_count += 1
...
def compute_embeddings_df(
    ...
    extractor.eval()
    x_all = df[input_feature_cols].reset_index(drop=True)
    chunks = []
...
def fit_fold(
    ...
    if should_run_pretrain:
        print(f"[fold {fold_id}] pretraining multitask contrastive learner with aux feature targets...")
        pretrain_one_fold(train_df, val_df, input_feature_cols, aux_target_cols, cfg, pretrain_dir, device)
    else:
        print(f"[fold {fold_id}] skip pretraining, reuse checkpoint: {pretrain_dir}")

    print(f"[fold {fold_id}] extracting pretrained embeddings...")
    extractor = PretrainedEmbeddingExtractor(input_feature_cols, str(pretrain_dir), cfg, device)

    train_emb = compute_embeddings_df(extractor, train_df, input_feature_cols, cfg.eval_batch_size)
    val_emb = compute_embeddings_df(extractor, val_df, input_feature_cols, cfg.eval_batch_size)
    test_emb = compute_embeddings_df(extractor, test_df, input_feature_cols, cfg.eval_batch_size)
    ...
    train_loader = DataLoader(EmbeddingTargetDataset(train_emb, train_y), batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.num_workers, pin_memory=False)
    val_loader = DataLoader(EmbeddingTargetDataset(val_emb, val_y), batch_size=cfg.eval_batch_size, shuffle=False, num_workers=cfg.num_workers, pin_memory=False)
    test_loader = DataLoader(EmbeddingTargetDataset(test_emb, test_y), batch_size=cfg.eval_batch_size, shuffle=False, num_workers=cfg.num_workers, pin_memory=False)

    gene_head = MLPHead(in_dim=train_emb.shape[1], hidden_dim=cfg.hidden_dim, out_dim=len(gene_cols), dropout=cfg.dropout).to(device)
    ...
    for epoch in range(cfg.max_epochs):
        train_loss = run_head_epoch(gene_head, train_loader, device, optimizer)
        val_loss = run_head_epoch(gene_head, val_loader, device, None)
    ...
    val_metrics = compute_regression_metrics(val_pred, val_true)
    test_metrics = compute_regression_metrics(test_pred, test_true)
    ...
    summary = {
        ...
        "embedding_dim": int(train_emb.shape[1]),
        "num_input_features": len(input_feature_cols),
...
def main() -> None:
    ...
    joint_df, all_feature_cols, input_feature_cols, aux_target_cols, gene_cols = build_joint_dataframe(cfg)
    joint_df.to_csv(output_dir / "joint_aligned_dataset.csv", index=False)
    with open(output_dir / "column_split.json", "w", encoding="utf-8") as f:
        json.dump(
            {
