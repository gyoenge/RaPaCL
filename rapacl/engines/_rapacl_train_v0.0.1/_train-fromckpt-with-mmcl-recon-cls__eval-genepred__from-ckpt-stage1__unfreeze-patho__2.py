...
import os
from dataclasses import datacllass
...
WARMUP_RECON_EPOCHS = 5 # 5
MMCL_LAMBDA = 1.0 # 0.3
RECON_LAMBDA = 1.0 # 1.0
CLS_LAMBDA = 1.0 # 1.0 # 0.5
...
def build_scratch_radiomics_model(device: torch.device):
    return build_radiomics_learner(
        ...
        device=device,
        num_sub_cols=NUM_SUB_COLS # getattr(train, "NUM_SUB_COLS", [72, 54, 36, 18, 9, 3, 1]),
    ).to(device)

def build_radiomics_model(device: torch.device):
    rad_ckpt = "/root/workspace/RaPaCL/rapacl/checkpoints/radiomics_retrieval/transtab" # getattr(constants, "RAD_TRANSTAB_CHECKPOINT_PATH", None)

    if rad_ckpt is not None:
        print(f"[INFO] load rad TransTab checkpoint: {rad_ckpt}")

    model = build_radiomics_learner(
        checkpoint=None,  # 핵심 변경
        numerical_columns=RADIOMICS_FEATURES_NAMES,
        num_class=getattr(train, "NUM_CELLTYPE_CLASSES", 5),
        hidden_dropout_prob=train.DROPOUT,
        projection_dim=train.PROJECTION_DIM,
    ...
    def load_radiomics_backbone_except_clf(model: nn.Module, checkpoint_path: str, device: torch.device):
...
def main():
    ...
    # skip_stage1_if_loaded = False 
    run_stage1 = True # not (loaded_stage1 and skip_stage1_if_loaded)
    ...
    if run_stage1:
        ...
        stage1_epochs = 20 # getattr(constants, "PRETRAIN_EPOCHS", getattr(constants, "EPOCHS", 50))
